...
{
    public class ObtemPedidosPorStatusUseCaseTest
    {
        [Fact]
        public async void Deve_Listar_Pedidos_Pagos()
        {
            //Arrange
            ...
            ObtemPedidosPorStatusUseCase useCase = new ObtemPedidosPorStatusUseCase(mockRepository.Object);


            //Act
            var pedidosNaoFinalizados = await useCase.Execute(EnumStatusPedido.Pago);

            //Assert
            Assert.NotNull(pedidosNaoFinalizados);
            Assert.Equal(3, pedidosNaoFinalizados.Count);
        }
        ...
        private List<Pedido> GerarPedidos()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                var quantidadeCheckout = 1;
                if (i != 0 && i != 3)
                    quantidadeCheckout = 3;
